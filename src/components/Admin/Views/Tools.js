...
const Tools = () => {
  ...
  const handleToggle = async (id) => {
    try {
      await adminApi.toggleTool(token, id);
      fetchTools();
    } catch (err) {
      alert(err.message);
    }
            ...
            {loading ? (
              ...
              tools.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>{t.orderIndex}</td>
                  <td>
                    <span className={`admin-badge ${t.isActive ? 'active' : 'inactive'}`}>
                      {t.isActive ? 'Bật' : 'Tắt'}
                    </span>
                  </td>
                  <td>
                    <div style={{ display: 'flex', gap: '5px' }}>
                      ...
                      >
                        {t.isActive ? (
                          <ToggleRight size={18} color="var(--admin-success)" />
                        ) : (
                          <ToggleLeft size={18} />
                        )}
                      </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div>
            ...
            <input
              ...
              placeholder="VD: Subnet Calculator"
          ...
          <div>
            ...
            <textarea
              className="admin-search-input"
              style={{ width: '100%', boxSizing: 'border-box', height: '60px', resize: 'vertical' }}
