...
const ReviewExam = () => {
  ...
  const userAnswers =
    result.userAnswers && Object.keys(result.userAnswers).length
      ? result.userAnswers
      : { 1: 'A', 2: 'C', 3: 'B' };
  ...
  const isCorrect =
    userAns.length === correctAns.length && userAns.every((val) => correctAns.includes(val));
  ...
  return (
    <div className="review-exam-page">
      ...
      <div className="take-exam__body">
        {!question ? (
          <div
            className="take-exam__question-card"
          ...
          <div className="take-exam__question-card">
            ...
            <div className="take-exam__question-meta">
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                ...
                >
                  {isCorrect ? '✓ CÂU HỎI ' : '✕ CÂU HỎI '}
                  {currentQ + 1}: {isCorrect ? 'ĐÚNG' : 'SAI'}
                </span>
                <span style={{ fontSize: '0.78rem', color: '#64748b' }}>
                  Lĩnh vực: {question.domain}
                </span>
              </div>
            </div>
            ...
            <div className="review-exam__explanation">
              ...
              <div className="review-exam__explanation-body">
                ...
                {question.explanationBullets && (
                  <ul>
                    {question.explanationBullets.map((b, i) => (
                      <li key={i}>{b}</li>
                    ))}
                  </ul>
                )}
            ...
            <div className="take-exam__nav">
              ...
              <span className="take-exam__nav-info">
                CÂU {currentQ + 1} TRÊN {questions.length}
              </span>
              <button
          ...
          >
            ...
            <div style={{ textAlign: 'center', padding: '0.5rem 0' }}>
              ...
              <div style={{ fontSize: '0.85rem', color: '#475569', marginTop: '0.3rem' }}>
                {answeredCorrect} / {questions.length} câu đúng
              </div>
            </div>
            ...
            <div style={{ marginTop: '0.5rem' }}>
              ...
              <div
                className="take-exam__progress-bar"
                style={{ height: '6px', background: '#e2e8f0' }}
              >
                <div
                  className="take-exam__progress-fill"
                  style={{ width: `${reviewPct}%`, background: '#2563eb' }}
                />
              </div>
            </div>
          </div>
          ...
          <div className="take-exam__sidebar-card">
            ...
            <div className="take-exam__q-grid">
              {questions.map((q, index) => {
                ...
                const ca = Array.isArray(q.correctAnswer) ? q.correctAnswer : [q.correctAnswer];
                const isRightAnswer =
                  ua.length === ca.length && ua.every((val) => ca.includes(val));
                let bg = '#f1f5f9',
                  color = '#64748b',
                  border = 'none';
                if (index === currentQ) {
