...
export const adminApi = {
  ...
  getDashboardActivity: (token) =>
    request(
      ...
      { headers: getHeaders(token) },
      'Lỗi lấy hoạt động dashboard'
    ),

  getDashboardDistribution: (token) =>
    request(
      `${API_URL}/admin/dashboard/distribution`,
      { headers: getHeaders(token) },
      'Lỗi lấy phân bổ dashboard'
    ),

  getDashboardTrends: (token) =>
    request(
      `${API_URL}/admin/dashboard/trends`,
      { headers: getHeaders(token) },
      'Lỗi lấy xu hướng dashboard'
    ),

  getRecentStudents: (token) =>
    request(
      `${API_URL}/admin/dashboard/students`,
      { headers: getHeaders(token) },
      'Lỗi lấy danh sách học viên mới'
    ),
  ...
  getCourses: (token, page = 1) =>
    request(
      `${API_URL}/learning/courses?page=${page}`,
      { headers: getHeaders(token) },
  ...
  getLabs: (token, page = 1) =>
    request(
      `${API_URL}/learning/labs?page=${page}`,
      { headers: getHeaders(token) },
      'Lỗi lấy danh sách bài thực hành'
    ),

  createLab: (token, formData) =>
    request(
      `${API_URL}/learning/labs`,
      {
        method: 'POST',
        headers: getHeaders(token, true),
        body: formData,
  ...
  getModules: (token, courseId) =>
    request(
      `${API_URL}/learning/courses/${courseId}/modules`,
      { headers: getHeaders(token) },
      'Lỗi lấy danh sách chương'
    ),
  ...
  updateModule: (token, moduleId, payload) =>
    request(
      `${API_URL}/learning/modules/${moduleId}`,
      {
        method: 'PUT',
        headers: getHeaders(token),
        body: JSON.stringify(payload),
  ...
  getLessons: (token, moduleId) =>
    request(
      `${API_URL}/learning/modules/${moduleId}/lessons`,
      { headers: getHeaders(token) },
      'Lỗi lấy danh sách bài học'
    ),
  ...
  updateLesson: (token, lessonId, payload) =>
    request(
      `${API_URL}/learning/lessons/${lessonId}`,
      {
        method: 'PUT',
        headers: getHeaders(token),
        body: JSON.stringify(payload),
  ...
  getExamById: (token, id) =>
    request(
      `${API_URL}/exams/detail/${id}`,
      { headers: getHeaders(token) },
      'Lỗi lấy chi tiết bài kiểm tra'
    ),
  ...
  getExamResults: (token, page = 1) =>
    request(
      `${API_URL}/exams/results?page=${page}`,
      { headers: getHeaders(token) },
      'Lỗi lấy kết quả thi'
    ),
  ...
  getTopics: (token, courseId) =>
    request(
      `${API_URL}/learning/courses/${courseId}/topics`,
      { headers: getHeaders(token) },
      'Lỗi lấy chủ đề'
    ),
  ...
  deleteTopic: (token, topicId) =>
    request(
      `${API_URL}/learning/topics/${topicId}`,
      {
        method: 'DELETE',
        headers: getHeaders(token),
      },
  ...
  getResources: (token, courseId = '', page = 1) => {
    ...
    return request(
      `${API_URL}/learning/resources?${params.toString()}`,
      { headers: getHeaders(token) },
      'Lỗi lấy tài liệu'
    );
  },
