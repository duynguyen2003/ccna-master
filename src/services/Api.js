...
export const api = {
  ...
  validateResetPasswordToken: (token) => apiFetch(`/auth/reset-password/${token}/validate`),
  ...
  cliAction: async (token, id, action, expectedRevision) =>
    (
      await apiFetch(`/lab-attempts/${id}/actions`, token, {
        method: 'POST',
        body: JSON.stringify({ action, expectedRevision }),
      })
    ).data,
  cliReplay: async (token, id, sequence) =>
    (await apiFetch(`/lab-attempts/${id}/replay?sequence=${sequence}`, token)).data,
  cliMembers: async (token, id, userId, remove = false) =>
    (
      await apiFetch(`/lab-attempts/${id}/members`, token, {
        method: 'POST',
        body: JSON.stringify({ userId, remove }),
      })
    ).data,
  cliExplain: async (token, id) =>
    (await apiFetch(`/lab-attempts/${id}/explain`, token, { method: 'POST' })).data,
  ...
  getStudyTimeStats: async (token, period = 'week') => {
    const json = await safeApiFetch(`/users/stats/study-time?period=${period}`, token, {
      data: [],
    });
    return json.data || [];
  },
  ...
  getExams: async (token) => {
    ...
    return (json.data || []).filter((exam) => exam.status === 'OPEN').map(mapExam);
  },
