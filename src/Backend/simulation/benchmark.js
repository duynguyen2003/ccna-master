...
const devices = Array.from({ length: 24 }, (_, i) => ({
  id: `N${i}`,
  deviceType: i === 0 || i === 23 ? 'PC' : 'SWITCH',
  interfaces:
    i === 0 || i === 23
      ? [
          {
            name: 'GigabitEthernet0/0',
            ipAddress: i === 0 ? '10.0.0.1' : '10.0.0.2',
...
const links = Array.from({ length: 23 }, (_, i) => ({
  id: `L${i}`,
  a: endpoint(`N${i}`, i === 0 ? 'GigabitEthernet0/0' : 'GigabitEthernet0/1'),
  b: endpoint(`N${i + 1}`, 'GigabitEthernet0/0'),
  enabled: true,
}));
  ...
  JSON.stringify(
    {
      ...
      links: 23,
      packets: 500,
      p50Ms: +samples[250].toFixed(3),
      p95Ms: +samples[475].toFixed(3),
      maxMs: +samples[499].toFixed(3),
